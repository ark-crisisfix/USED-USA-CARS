  ...
  return (
    <div className="bg-gray-50 min-h-screen py-16 px-4 font-sans">
      <div className="max-w-6xl mx-auto">
        ...
        <p className="text-xl text-gray-600 mb-16 text-center max-w-2xl mx-auto">{t.subtitle}</p>
            ...
            >
              <div className="md:w-2/5 h-64 md:h-auto relative">
                ...
                <div className="absolute top-4 left-4 bg-emerald-500 text-white font-bold px-3 py-1 rounded-full text-sm shadow-md">
                  {t.saved} ${c.savings.toLocaleString()}
                </div>
              </div>
              <div className="p-8 md:w-3/5">
                <div className="flex justify-between items-start mb-6">
                  <div>
                    ...
                    <p className="text-gray-500">
                      {t.deliveredTo} <span className="font-medium text-gray-800">{c.destination}</span>
                    </p>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-y-4 gap-x-8 mb-8 text-sm">
                  <div>
                    <div className="text-gray-500 mb-1">{t.winningBid}</div>
                    <div className="font-semibold text-gray-900 text-lg">${c.auctionPrice.toLocaleString()}</div>
                  </div>
                  <div>
                    <div className="text-gray-500 mb-1">{t.totalToPort}</div>
                    <div className="font-semibold text-gray-900 text-lg">${c.totalCostToPort.toLocaleString()}</div>
                  </div>
                  <div>
                    <div className="text-gray-500 mb-1">{t.estRepair}</div>
                    <div className="font-semibold text-gray-900 text-lg">${c.repairCost.toLocaleString()}</div>
                  </div>
                  <div>
                    <div className="text-gray-500 mb-1">{t.localMarket}</div>
                    <div className="font-bold text-red-600 text-lg line-through decoration-2">
                      ${c.localMarketPrice.toLocaleString()}
                    </div>
                  </div>
                </div>
                ...
                <div className="border-t border-gray-100 pt-6">
                  <div className="flex items-center justify-between">
                    <div>
                      <div className="text-sm text-gray-500 mb-1">{t.finalCost}</div>
                      <div className="text-2xl font-black text-gray-900">
                        ${(c.totalCostToPort + c.repairCost).toLocaleString()}
                      </div>
                    </div>
                    <Link
                      href={L("/contact")}
